...
シフト管理は ADR 0014 により Phase 2 以降とする。
Phase 2 では **人が作り、人が修正し、人が確認できるシフト表管理** を成立させる。
...
Phase 2 の初期実装ではシフト変更の専用ログテーブルは作らない。
...
差分検出結果は LINE WORKS 通知の候補。Phase 2 では以下を送る:
...
- Phase 2 の初期実装では `ShiftChangeLog` は導入しない
