...
- 打刻端末でカードをかざす
- 確認 UI で種別を確認 / 変更
- LINE WORKS で自分の勤怠 / シフトを照会
- LINE WORKS で打刻漏れ・修正を申請
...
3. Server が「未登録」を返し、audit log に記録、LINE WORKS で管理者に通知
4. Terminal は「このカードは登録されていません。管理者にお問い合わせください」と表示
5. 管理者が Admin Web から card binding 画面で従業員に紐付ける

### UC-03: LINE WORKS 修正申請
