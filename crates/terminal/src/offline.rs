...
impl OfflineRepository {
    ...
    pub async fn save_punch(&self, id: Uuid, card_id: &str, event_type: &str, occurred_at: &Zoned) -> Result<()> {
        sqlx::query(
            "INSERT INTO local_punches (id, card_id, event_type, occurred_at) VALUES (?, ?, ?, ?)"
        )
        .bind(id.to_string())
        .bind(card_id)
        .bind(event_type)
    ...
    pub async fn mark_as_synced(&self, id: Uuid, synced_at: &Zoned) -> Result<()> {
        sqlx::query(
            "UPDATE local_punches SET synced_at = ? WHERE id = ?"
        )
    ...
    pub async fn get_unsynced_punches(&self) -> Result<Vec<LocalPunch>> {
        let rows = sqlx::query_as::<_, (String, String, String, String, Option<String>)>(
            "SELECT id, card_id, event_type, occurred_at, synced_at FROM local_punches WHERE synced_at IS NULL"
        )
        .fetch_all(&self.pool)
        .await?;
        ...
        for row in rows {
            punches.push(LocalPunch {
                ...
                _synced_at: row.4.map(|s| Zoned::from_str(&s)).transpose()?,
            });
        }
